...
function JobListPage() {
  ...
  return (
    ...
    >
      ...
      <Box mt="25px">
        <Box display="flex" flexDirection="row" gap="30px">
          ...
          <Button sx={TabButtonStyle()} onClick={() => setIsDraftPage(false)}>
            <Typography sx={TabTextStyle(isDraftPage === false)}>
              Công việc đã tạo
            </Typography>
          </Button>
          <Divider orientation="vertical" flexItem />
          <Button sx={TabButtonStyle()} onClick={() => setIsDraftPage(true)}>
            <Typography sx={TabTextStyle(isDraftPage === true)}>
              Bản nháp
            </Typography>
          </Button>
        </Box>
      ...
      >
        {!isDraftPage ? (
          <CreateJobTable data={jobListCreatedData} />
        ) : (
          <DraftJobTable data={jobListDraftData} />
        )}
        <Box
          sx={{
            marginTop: "25px",
